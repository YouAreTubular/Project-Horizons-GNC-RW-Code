...
// =====================================================
// USB + BLUETOOTH MIRROR PRINTING
// =====================================================

// Anything printed with these macros will go to BOTH:
// 1. USB Serial Monitor at 115200 baud
// 2. Bluetooth serial at 9600 baud

#define BOTH_PRINT(x)        do { Serial.print(x); BTSerial.print(x); } while (0)
#define BOTH_PRINTLN(x)      do { Serial.println(x); BTSerial.println(x); } while (0)
#define BOTH_PRINT_DEC(x, d) do { Serial.print(x, d); BTSerial.print(x, d); } while (0)

#define BOTH_PRINTLN_EMPTY() do { Serial.println(); BTSerial.println(); } while (0)

#define BOTH_WRITE_BUFFER(buf, len) do { \
  Serial.write((const uint8_t*)(buf), (len)); \
  BTSerial.write((const uint8_t*)(buf), (len)); \
} while (0)

// Optional bridge:
// If the Bluetooth module sends anything back to the MCU,
// this lets you see it in the USB Serial Monitor.
void bridgeBluetoothToUSB();

// Optional bridge:
// If you type into the USB Serial Monitor,
// this forwards that text to the Bluetooth module.
void bridgeUSBToBluetooth();
...
// Tracks where the last Bluetooth/USB transmission stopped.
...
void transmitFlashLogOverBluetoothAndUSB();
...
void setup(void) {
  ...
  BOTH_PRINTLN("System Booting...");
  BOTH_PRINTLN("USB Serial Monitor mirrors Bluetooth output.");
  BOTH_PRINTLN("Open Serial Monitor at 115200 baud.");
  ...
  BOTH_PRINTLN("Calibrating Gyro...");

  calibrategyro();

  BOTH_PRINTLN("Calibration Complete.");
...
void loop() {
  // Optional live bridge for anything received over USB/Bluetooth.
  bridgeBluetoothToUSB();
  bridgeUSBToBluetooth();
  ...
  switch (state) {
    case prelaunch:
      if (millis() - last_time > 1000) {
        BOTH_PRINTLN("STATE: PRELAUNCH");

        BOTH_PRINT("Acc[X,Y,Z]: [");
        BOTH_PRINT(accel.acceleration.x);
        BOTH_PRINT(", ");
        BOTH_PRINT(accel.acceleration.y);
        BOTH_PRINT(", ");
        BOTH_PRINT(accel.acceleration.z);
        BOTH_PRINT("]\t");

        BOTH_PRINT("Mag: ");
        BOTH_PRINT(accel_mag);
        BOTH_PRINT("\t");

        BOTH_PRINT("Temp: ");
        BOTH_PRINT(temp.temperature);
        BOTH_PRINT(" C\t");
      ...
      if (fabs(accel_mag - 9.81f) > 2.0f) {
        BOTH_PRINT("[FLAG: BAD_PAD_CALIBRATION] ");
      }

      if (accel_mag >= launch_thresh) {
        state = launch;

        BOTH_PRINTLN("LAUNCH DETECTED!");
      }
      break;
  ...
  // AND mirror that exact same output to USB Serial Monitor.
  if (millis() - lastTransmitTime >= TRANSMIT_INTERVAL_MS) {
    transmitFlashLogOverBluetoothAndUSB();
...
// =====================================================
// USB / BLUETOOTH BRIDGE FUNCTIONS
// =====================================================
...
void setupFlashStorage() {
  BOTH_PRINTLN("Initializing W25Q64JV SPI flash...");
  ...
  if (!flash.begin()) {
    ...
    BOTH_PRINTLN("FLASH CHIP FAILED OR NOT PRESENT");
    BOTH_PRINTLN("Check wiring: VCC=3.3V, GND, PA5 SCK, PA6 MISO, PA7 MOSI, PA4 CS.");

    return;
  }

  BOTH_PRINTLN("FLASH CHIP FOUND");

  // Step 2: Check if a FAT filesystem exists.
  if (!fatfs.begin(&flash)) {
    flashStatus = FLASH_UNFORMATTED;

    BOTH_PRINTLN("FLASH CHIP FOUND, BUT FILESYSTEM FAILED");
    BOTH_PRINTLN("FLASH IS PROBABLY UNFORMATTED");
    BOTH_PRINTLN("Run Adafruit SPIFlash > fatfs_format first.");
    BOTH_PRINTLN("Then re-upload this reaction wheel code.");
  ...
  BOTH_PRINTLN("FLASH FILESYSTEM READY");

  if (!openTelemetryFileForAppend()) {
    flashStatus = FLASH_UNFORMATTED;

    BOTH_PRINTLN("Could not open imu.csv even though filesystem mounted.");
    BOTH_PRINTLN("Flash may need to be reformatted.");

    return;
  }

  BOTH_PRINTLN("Flash logging ready.");
}
...
bool openTelemetryFileForAppend() {
  ...
  if (telemetryFile.size() == 0) {
    ...
    // First Bluetooth/USB transmission should include the header.
    lastTransmitPosition = 0;
  } else {
    // Existing file from previous boot.
    // Start transmission from the current end of file,
...
void transmitFlashLogOverBluetoothAndUSB() {
  ...
  if (!readFile) {
    BOTH_PRINTLN("Could not open imu.csv for Bluetooth/USB transmission.");
  ...
  // If no new data was added, do not spam Bluetooth/USB.
  if (lastTransmitPosition == fileSize) {
    BOTH_PRINTLN_EMPTY();
    BOTH_PRINTLN("No new flash IMU data to transmit.");
    BOTH_PRINTLN_EMPTY();
  ...
  BOTH_PRINTLN_EMPTY();
  BOTH_PRINTLN("===== BEGIN NEW FLASH IMU DATA =====");
  ...
  while (readFile.available()) {
    ...
    if (bytesRead > 0) {
      BOTH_WRITE_BUFFER(buffer, bytesRead);
    }
  }
  ...
  BOTH_PRINTLN_EMPTY();
  BOTH_PRINTLN("===== END NEW FLASH IMU DATA =====");
  BOTH_PRINTLN_EMPTY();

  BOTH_PRINTLN("New flash IMU data transmitted over Bluetooth and mirrored to USB.");
...
void printFlashProblemMessage() {
  if (flashStatus == FLASH_NOT_FOUND) {
    BOTH_PRINTLN("FLASH ERROR: W25Q64JV not detected.");
    BOTH_PRINTLN("Check 3.3V power, GND, PA5 SCK, PA6 MISO, PA7 MOSI, PA4 CS.");
  }

  else if (flashStatus == FLASH_UNFORMATTED) {
    BOTH_PRINTLN("FLASH ERROR: W25Q64JV detected but not FAT-formatted.");
    BOTH_PRINTLN("Run Adafruit SPIFlash > fatfs_format first.");
  }

  else if (flashStatus == FLASH_NOT_CHECKED) {
    BOTH_PRINTLN("FLASH ERROR: Flash status was not checked.");
  }
}